...
// For texturing
void Plane::getTexCoord(glm::vec3 p, float& texcoords, float& texcoordt)
{
	float textures;
	float texturet;

	
	if (a_.y == b_.y && a_.y == c_.y && a_.y == d_.y)
	{
		// flat plane
		textures = (p.x - a_.x) / (b_.x - a_.x);
		texturet = (p.z - a_.z) / (d_.z - a_.z);
	}
	else
	{	
		// verticle plane
		textures = (p.x - a_.x) / (b_.x - a_.x);
		texturet = (p.y - a_.y) / (d_.y - a_.y);
	}
